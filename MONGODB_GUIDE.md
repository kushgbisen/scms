...
This guide demonstrates the database structure and MongoDB shell commands for populating the student course management database.
...
```javascript
{
  ...
  "email": "emma.smith12@university.edu",
...
## MongoDB Shell Commands for Database Population

### Connect to MongoDB
```bash
mongosh "mongodb://localhost:27017/student_course_db"
```
...
// Find all courses taught by INS001
db.courses.find({ instructor_id: "INS001" })

// Find all enrollments for a specific student
db.enrollments.find({ student_id: "CS10001" }).pretty()

// Find all students in a specific course
db.enrollments.find({ course_code: "CS101" })

// Aggregate: Calculate average GPA by major
db.students.aggregate([
  { $group: { _id: "$major", avgGpa: { $avg: "$gpa" }, count: { $sum: 1 } } },
  { $sort: { avgGpa: -1 } }
])

// Aggregate: Course enrollment statistics
db.enrollments.aggregate([
  { $group: { _id: "$course_code", enrolled: { $sum: 1 } } },
  { $sort: { enrolled: -1 } }
])
```
...
You can create a complete setup script by combining all commands:

```bash
# Create setup.js file with all the MongoDB commands above
cat > setup.js << 'EOF'
...
// Insert all data (copy the insertMany statements from above)
// [Insert instructors, courses, students, and enrollments]
...
mongosh "mongodb://localhost:27017/student_course_db" setup.js
```

This guide provides both the theoretical database structure and practical MongoDB shell commands to populate your student course management database.
